...
describe('ShellTool', () => {
  ...
  describe('getCommandRoot', () => {
    ...
    it('should extract the command root from a command with windows path', () => {
      expect(shellTool.getCommandRoot('C:\\Windows\\System32\\cmd.exe /c echo hello')).toBe('cmd.exe');
    });

    it('should handle commands with pipes and redirects', () => {
      expect(shellTool.getCommandRoot('grep "foo" | wc -l > count.txt')).toBe('grep');
    });
  ...
  describe('getDescription', () => {
    ...
    it('should generate a description for a command with a directory', () => {
      expect(shellTool.getDescription({ command: 'ls -la', directory: 'src' })).toBe('ls -la [in src]');
    });

    it('should generate a description for a command with a description', () => {
      expect(shellTool.getDescription({ command: 'ls -la', description: 'list files' })).toBe('ls -la (list files)');
    });

    it('should generate a description for a command with a directory and a description', () => {
      expect(shellTool.getDescription({ command: 'ls -la', directory: 'src', description: 'list files' })).toBe('ls -la [in src] (list files)');
    });

    it('should handle newlines in the description', () => {
        expect(shellTool.getDescription({ command: 'ls', description: 'a\nb' })).toBe('ls (a b)');
    });
  });
  ...
  describe('validateToolParams', () => {
    ...
    it('should return an error for an absolute directory path', () => {
      expect(shellTool.validateToolParams({ command: 'ls', directory: '/abs/path' })).toBe('Directory cannot be absolute. Must be relative to the project root directory.');
    });

    it('should return an error for a non-existent directory', () => {
        expect(shellTool.validateToolParams({ command: 'ls', directory: 'non-existent-dir' })).toBe('Directory must exist.');
    });

    it('should return null for a valid directory', () => {
        const validDir = 'src';
        fs.mkdirSync(path.join(tempDir, validDir), { recursive: true });
        expect(shellTool.validateToolParams({ command: 'ls', directory: validDir })).toBeNull();
    });
  });

  describe('shouldConfirmExecute', () => {
    it('should return false if params are invalid', async () => {
        const result = await shellTool.shouldConfirmExecute({ command: ' ' }, new AbortController().signal);
        expect(result).toBe(false);
    });

    it('should return false if the command is whitelisted', async () => {
        const params = { command: 'ls -la' };
        const confirmationDetails = await shellTool.shouldConfirmExecute(params, new AbortController().signal);
        if (typeof confirmationDetails !== 'boolean') {
            await confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedAlways);
        }

        const result = await shellTool.shouldConfirmExecute(params, new AbortController().signal);
        expect(result).toBe(false);
    });

    it('should return confirmation details if the command is not whitelisted', async () => {
        const result = await shellTool.shouldConfirmExecute({ command: 'ls -la' }, new AbortController().signal);
  ...
  describe('execute', () => {
    ...
    it('should execute a command successfully', async () => {
        const result = await shellTool.execute({ command: 'echo "hello"' }, new AbortController().signal);
    ...
    it('should handle command with stderr output', async () => {
        mockSpawn.on = vi.fn((event: string, cb: (code: number | null, signal: string | null) => void) => {
            if (event === 'exit') {
                setTimeout(() => cb(1, null), 10);
            }
        });
        mockSpawn.stderr.on.mockImplementation((event: string, cb: (data: Buffer) => void) => {
            if (event === 'data') {
                cb(Buffer.from('error message'));
            }
        });

        const result = await shellTool.execute({ command: 'error_command' }, new AbortController().signal);
    ...
    it('should handle command failure', async () => {
        ...
        mockSpawn.on = vi.fn((event: string, cb: (error: Error | null) => void) => {
            if (event === 'error') {
                cb(error);
            }
            if (event === 'exit') {
        ...
        const result = await shellTool.execute({ command: 'fail_command' }, new AbortController().signal);
        expect(result.llmContent).toContain('Error: Error: spawn error');
    });

    it('should handle command cancellation', async () => {
        const abortController = new AbortController();
        const promise = shellTool.execute({ command: 'long_running_command' }, abortController.signal);
    ...
    it('should execute command in the specified directory', async () => {
        ...
        await shellTool.execute({ command: 'ls', directory: dir }, new AbortController().signal);
