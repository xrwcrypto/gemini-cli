...
describe('EditTool', () => {
  ...
  describe('getModifyContext', () => {
    ...
    it('getFilePath should return the file_path from params', () => {
      const context = tool.getModifyContext(new AbortController().signal);
      const params = {
        file_path: filePath,
        old_string: 'old',
        new_string: 'new',
      };
      expect(context.getFilePath(params)).toBe(filePath);
    });

    it('getCurrentContent should read the file content', async () => {
      const fileContent = 'This is the current content.';
      ...
      await expect(context.getCurrentContent(params)).resolves.toBe(
        fileContent,
      );
    });

    it('getCurrentContent should return empty string for non-existent file', async () => {
      const context = tool.getModifyContext(new AbortController().signal);
      const params = {
        file_path: filePath,
        old_string: 'old',
    ...
    it('getProposedContent should return the content after replacement', async () => {
      const fileContent = 'replace this old string';
      fs.writeFileSync(filePath, fileContent, 'utf8');
      const context = tool.getModifyContext(new AbortController().signal);
      const params = {
        file_path: filePath,
      ...
      await expect(context.getProposedContent(params)).resolves.toBe(
        'replace this new string',
      );
    });

    it('getProposedContent should return empty string for non-existent file', async () => {
      const context = tool.getModifyContext(new AbortController().signal);
      const params = {
        file_path: filePath,
        old_string: 'old',
    ...
    it('createUpdatedParams should return updated params', () => {
      const context = tool.getModifyContext(new AbortController().signal);
