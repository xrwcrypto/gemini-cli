...
This section provides guidance for contributing to the WebRun feature, which allows deploying Gemini CLI agents to Google Cloud Run.

**Recommended Development Environment:**

The recommended environment for WebRun development is **Google Cloud Shell**. It comes pre-installed with all the necessary tools (Docker, gcloud CLI, etc.) and provides a secure, authenticated environment.

A key advantage of Cloud Shell is its "Web Preview" feature. The WebRun frontend requires a secure (HTTPS) endpoint for OAuth to function correctly, and Web Preview provides this out of the box, simplifying the development and testing process.
...
2.  **Enable Sandbox for Build:**
    The WebRun container (`webrun.Dockerfile`) depends on the sandbox image. You must enable sandbox building by setting the `GEMINI_SANDBOX` environment variable.

3.  **Build All Components:**
    Run the following command to build the CLI, the sandbox, and the WebRun container:
    ```bash
    npm run build:all
    ```
...
To serve the frontend web application locally, run:

```bash
npm run serve:web
```

If you are using Cloud Shell, you can then use the Web Preview feature to access the running application over HTTPS.

**OAuth Setup (Manual):**

Currently, the OAuth configuration for the WebRun frontend is managed manually. To test the application, you will need to:

1.  Be added as a test user to the OAuth client.
2.  Have your Cloud Shell's Web Preview URL added to the list of authorized redirect URIs.

Please contact `jmahood@google.com` or `adamkane@google.com` to get set up. Provide them with your Google account email and the HTTPS URL generated by the Cloud Shell Web Preview.

**Testing the WebRun Container Locally:**

You can test the WebRun container locally using Docker.

1.  **Build the container** by running `npm run build:all` as described above.

2.  **Run the container:**
    ```bash
    docker run -p 8080:8080 us-west1-docker.pkg.dev/gemini-run/containers/gemini-cli-webrun:dev
    ```
    This command starts the container and maps port 8080 in the container to port 8080 on your local machine.
